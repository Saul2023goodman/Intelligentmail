...
Comments must be settled before the scheduling path can be exercised live.
...
- [ ] Add the Execution Run and Execution Run Item terms to the domain glossary: one operator-
...
- [ ] Persist runs in `schema.sql`: `execution_runs(id, campaign_id, kind, state, requested_count,
...
- [ ] Every run belongs to exactly one Campaign and has exactly one `kind` (`immediate`, `scheduled`,
      `cancellation`, `replacement`); a mixed-kind request is refused before any external action.
- [ ] A run is created only by an explicit operator request; nothing starts, resumes, or retries a run
      automatically, and observing a run never mutates the mailbox.

### One authorization, one run

- [ ] Add `run_batch(confirmation_ids, *, kind=None)` at the Core boundary: validate the whole set
...
- [ ] Each Confirmation is executed by its own existing single-action path (immediate send, native
...
- [ ] A run that reaches every item becomes `completed`; `executed_count` counts only reached items.
- [ ] `execution_confirm` returns the Confirmations it created so the batch page can carry them
...
- [ ] A `not_reached` item stays eligible for a later, explicitly requested run; a reached item is
      never re-executed by a subsequent run, and an already-sent Preparation is refused as before.
- [ ] A run interrupted by restart stays `running` until it is reconciled: recovery re-establishes
...
- [ ] Run history is inspectable after the Campaign's panels have emptied, so a finished batch is
      still answerable ("what did that run actually do") without re-deriving it from raw attempts.

### Core-computed execution queue

- [ ] Add an `execution_queue(campaign_id)` query returning one row per active Preparation with its
...
- [ ] The batch page derives its queue and its "awaiting execution" groups from this query instead of
      assembling them locally from reviews, schedules, and confirmations.

### Frontend: one batch flow

- [ ] One selection only, defaulting to every `ready_to_authorize` Preparation, so the normal path is
      subtraction (deselect the one or two to hold back) rather than building a selection from empty.
- [ ] Primary action "Review and send N": one review dialog, one authorization, then the run, with
      visible progress and a result summary (sent / failed / unknown / not reached).
- [ ] Secondary action "Authorize only" keeps the separated two-step path available for an operator
...
- [ ] Authorize and run for immediate and for scheduled actions live in distinct regions, each stating
      its scope, so a Campaign-wide plan confirmation can no longer be mistaken for the current
      selection's confirmation.
- [ ] A stopped run shows where it stopped, how many items were never reached, the pause reason, and a
...
- [ ] Availability is reported per kind, so a disabled kind blocks only its own region and says why.

### Scheduling stays a first-class path

- [ ] Propose, adjust, and confirm a Sending Plan as today; confirming a plan yields scheduled
      Confirmations that appear as an awaiting-execution group of kind `scheduled`.
- [ ] Placing those schedules is one run of kind `scheduled`, with the same progress, stopping, and
      `not_reached` semantics; a scheduled Confirmation is never executed as an immediate send.
- [ ] Native scheduling availability is reported independently of immediate sending, and the
...
- [ ] The existing safeguards still hold inside a batch: content and attachment digests, Confirmation
...
- [ ] The full Core test suite passes, and every Core behaviour above is covered at the command/query
