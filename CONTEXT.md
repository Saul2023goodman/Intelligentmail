...
**Follow-up Rule**:
A supported campaign rule defining follow-up timing and count limits, optionally paired with a template for automatic preparation.
_Avoid_: Automatic sending authorization
...
**Follow-up Action**:
A separate communication action linked to earlier outreach, prepared from a supported rule or template or by an operator, and requiring its own sending confirmation.
