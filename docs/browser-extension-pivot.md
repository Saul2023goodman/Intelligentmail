...
| 登录与切换账号 | 打开/重启受控浏览器并接管登录 | 操作员在自己的邮箱页面登录、完成验证；扩展自动连接唯一或前台邮箱页，歧义时才要求选择，账号不匹配时拒绝操作 |
...
5. 在浏览器正常登录 `https://mail.163.com` 并进入邮箱主页。扩展会自动发现并连接唯一的邮箱页；页面重载或本机桥接短暂断开后会自动恢复。只有同时存在多个后台邮箱页、无法唯一判断时，才需在目标页面打开扩展并点击“优先连接当前邮箱”。
6. 命令行使用与安装时相同的 `--home`：
...
SmartMail 核心 → 163-extension adapter → 本地命令队列
                                ...
                                Native Messaging host
                                          ↕
                            扩展 service worker（自动发现，歧义时显式选择）
                                          ↕
                              隔离脚本：只读采集 / 确认后撰写
```
