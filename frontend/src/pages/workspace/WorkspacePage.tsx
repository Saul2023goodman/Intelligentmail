...
export default function WorkspacePage({ route }: { route: Route }) {
  ...
  const create = async (event: React.SubmitEvent) => {
    ...
    setError("");
    try {
      const value = await core("create_campaign", { name });
  ...
  const check = async (id: string) => {
    ...
    setError("");
    try {
      const result = await core("check_duplicate", {
        preparation_id: id,
      });
      await load();
  ...
  // Remain mounted while Intake is active, preserving campaign and inspector state.
    ...
    >
      <div className="workspace">
        ...
        <div className="toolbar">
          ...
          >
            <Icon name="branch" size={16} /> Source mapping
          </button>
